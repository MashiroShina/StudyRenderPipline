...
VertexOutput ShadowCasterPassVertex (VertexInput input) {
	...
	#if UNITY_REVERSED_Z
	    output.clipPos.z -= _ShadowBias;//设置深度偏移
...
float4 ShadowCasterPassFragment (VertexOutput input) : SV_TARGET {
	...
	float4 alpha = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, input.uv);
	alpha *= UNITY_ACCESS_INSTANCED_PROP(PerInstance, _Color);
	clip(alpha - _Cutoff);
	#endif
	return 0;
}
