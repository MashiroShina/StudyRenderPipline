...
float3 DiffuseLight (int index, float3 normal, float3 worldPos, float shadowAttenuation) {
	...
	diffuse = saturate( diffuse* (shadowAttenuation * spotFade * rangeFade / distanceSqr));
	//======================================================================================
...
float3 MainLight (float3 normal, float3 worldPos){
    ...
    float diffuse = dot(normal, lightDirection);
    diffuse = saturate(diffuse* shadowAttenuation);
    return diffuse * lightColor;
}
//==========================================================================================
...
float4 o = pos * 0.5f;
o.xy = float2(o.x, o.y*_PojectMatrixForme.x) + o.w;
...
VertexOutput LitPassVertex (VertexInput input) {
	...
	//output.worldPos=output.screenPos;
...
float4 LitPassFragment (VertexOutput input, FRONT_FACE_TYPE isFrontFace : FRONT_FACE_SEMANTIC) : SV_TARGET {
    ...
    float halftoneValue = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, textureCoordinate).r;
    //float4 albedoAlpha = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, textureCoordinate);
